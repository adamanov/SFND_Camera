                      ...
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC)
{
    ...
    { // outer keypoint loop
        ...
        cv::KeyPoint kpOuterCurr = kptsCurr.at(it1->trainIdx);  // trainIdx  = train descriptor index
        cv::KeyPoint kpOuterPrev = kptsPrev.at(it1->queryIdx);  // queryIdx  = query descriptor index
    ...
    // Alternative to sort, but more faster
    ...
    if(size % 2 == 0)
    {
        meanDistRatio = (distRatios[size/ 2] + distRatios[size / 2 -1]) /2 ;

        return;
    }
    else
        {
        meanDistRatio = distRatios[size/2];
        }

    // Same,but in more beauty way to write it.
    //double medDistRatio = distRatios.size() % 2 == 0 ? (distRatios[floor(distRatios.size() / 2.0) - 1] + distRatios[floor(distRatios.size() / 2.0)]) / 2.0 : distRatios[floor(distRatios.size() / 2.0)]; // compute median dist. ratio to remove outlier influence
